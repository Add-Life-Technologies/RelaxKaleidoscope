...
namespace UtopiaWorx.Shaders.PostProcessing
{
	...
	public class HeliosEditor : Editor 
	{
		...
		public override void OnInspectorGUI()
		{
            ...
            bool RenderTexturesExist = System.IO.Directory.Exists(Application.dataPath + "/" + "UtopiaWorx/Helios/Cubemaps");
...
//				}
			...
			if(sp_FoldoutAdvanced.boolValue)
			{
                ...
                if (sp_GammaCorrect.boolValue == true)
                {
		...
		static void Command (string input, string RunPath, int CaptureRate, bool DeleteStills, bool Do3D, string SceneName, string AudioPath, bool DebugMode, float Duration, string Scale, string Preset, string FileFormat, string OutputExt, string TemplateName, string AdditionalSwitches)
		{
			...
			args = "-framerate " + CaptureRate.ToString() + " -y -i '" + input + TemplateName + "%05d." + FileFormat + "' " + AudioPath + " -c:v " + Encoder + " -r " + CaptureRate.ToString() + " -pix_fmt yuv420p " + Scale + " " + Preset + " -t " + Duration.ToString() + AdditionalSwitches +" '" + input + SceneName.Replace(" ","") + "." + OutputOverwrite +"'";
...
#if UNITY_EDITOR_WIN
            string args = "-framerate " + CaptureRate.ToString() + " -y -i \"" + input.Replace(@"/",@"\") + TemplateName + "%05d." + FileFormat + "\" " + AudioPath.Replace(@"/",@"\") + " -c:v " + Encoder + " -r " + CaptureRate.ToString() + " -pix_fmt yuv420p " + Scale + " " + Preset + " -t "+ Duration.ToString() + AdditionalSwitches + " \""  + input.Replace(@"/",@"\") + SceneName.Replace(" ","") + "." + OutputOverwrite + "\"";
			if(DebugMode == true)
			{
				UnityEngine.Debug.Log(args);
			}
